...
import com.ssafy.api.dto.res.SignUpResDTO;
import com.ssafy.api.service.SignService;
import com.ssafy.core.code.JoinCode;
import com.ssafy.core.code.MFCode;
import com.ssafy.core.code.YNCode;
...
public class SignController {
    ...
    // 3) 회원가입
    @PostMapping("/regProfile")
    public SignUpResDTO signUp(
            @RequestBody @Valid SignUpReqDTO req
    ) {
        // DTO → Entity 매핑
        User u = User.builder()
                .uid(req.getUid())
//                .joinType(JoinCode.none)
                .joinType(JoinCode.fromValue(req.getType()))
//                .gender(MFCode.M)
//                .push(YNCode.N)
//                .isBind(YNCode.Y)
        ...
        long id = signService.userSignUp(u);
        return new SignUpResDTO(id, 1, "회원가입 성공");
    }

    // 4) 소셜가입 여부 확인
    @GetMapping("/exists/social")
    public GenericResponse existsSocial(
            @RequestParam String type,
            @RequestParam String uid
    ) {
        boolean ex = signService.userExists(
                uid,
                JoinCode.fromValue(type)
        );
    ...
    public LoginResDTO login(
            @RequestBody @Valid LoginUserReqDTO req
    ) {
        User user = signService.userLogin(
                req.getUid(),
                JoinCode.fromValue(req.getType()),
                req.getPassword()
        );
        // (선택) JWT 발급 로직이 있다면 토큰 꺼내 오세요
        String jwt = "";
        return new LoginResDTO(
                user.getId(),
                jwt,
                user.getName(), // or user.getNickname()
                1,
                "로그인 성공"
        );
    }
